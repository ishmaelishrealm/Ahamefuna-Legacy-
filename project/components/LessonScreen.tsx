...
export function LessonScreen({ 
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#FFB6D9] via-[#9D4EDD] to-[#00FF94]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            {getOptions().map((option, index) => {
              ...
              return (
                ...
                >
                  <span className={`text-2xl ${showCorrect || showIncorrect || isSelected ? 'text-white' : 'text-[#1A1A1A]'}`}>
                    {option}
                  </span>
                  {showCorrect && (
                  ...
                  {showIncorrect && (
                    <div className="bg-white rounded-full p-2">
                      <XCircle className="w-8 h-8 text-[#FF1493]" />
                    </div>
                  )}
                </button>
              );
      ...
      {/* Bottom Button Area */}
      <div className="bg-white/95 backdrop-blur-md game-border border-t-0 p-8 max-w-5xl mx-auto w-full">
        {!showFeedback ? (
          <button
            onClick={handleSubmit}
            disabled={!userAnswer}
            className={`
              w-full py-6 rounded-2xl game-border transition-all text-3xl uppercase tracking-wider retro-shadow-lg
              ${userAnswer
          ...
          <button
            onClick={handleNext}
            className="w-full py-6 rounded-2xl bg-gradient-to-r from-[#FFD700] to-[#FF6B35] text-white game-border retro-shadow-lg hover:scale-105 transition-all text-3xl uppercase tracking-wider"
          >
